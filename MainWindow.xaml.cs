    ...
    public partial class MainWindow : Window, INotifyPropertyChanged
    {
        ...
        private void ScreenshotButton_Click(object sender, RoutedEventArgs e)
        {
            if (this.depthBitmap != null)
            {
                ...
                // write the raw values to file
                try
                {
                    using (StreamWriter sw = new StreamWriter(rawDataPath))
                    {
                        foreach(ushort val in this.rawDepth)
                        {
                            sw.WriteLine(val);
                        }
                    }

                    this.StatusText = string.Format(CultureInfo.CurrentCulture, Properties.Resources.SavedScreenshotStatusTextFormat, rawDataPath);
                }

                catch (IOException)
                {
                    this.StatusText = string.Format(CultureInfo.CurrentCulture, Properties.Resources.FailedScreenshotStatusTextFormat, rawDataPath);
                    Directory.Delete(folderPath);
                }
            }
        }
